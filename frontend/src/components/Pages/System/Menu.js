...
const MenuLists = () => {
  ...
  const [menu, setMenu] = React.useState(initialState);
    ...
    fetch(backend_url + "addMenu", requestOptions)
      ...
      .then((data) => {
        if (data.code - 200 === 0) {
          ...
          const newItem = api.addItem(menu.name, true, rootNode);
  ...
  const updateElection = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(backend_url + "updateMenu", requestOptions)
          .then((response) => response.json())
          .then((data) => {
            ...
            if (data.code - 200 === 0) {
              ...
              Swal.fire(
                "Updated!",
                "Your file has been successfuly updated.",
                "success"
              );
              setMenu(initialState);
            }
      ...
      if (result.isConfirmed) {
        fetch(backend_url + "delMenu/?id=" + menu.id, { method: "GET" })
          ...
          .then((data) => {
            ...
            console.log("______", selectedItem);
  ...
  const setCheckMenu = () => {
    menu.parentId === 0
      ? setMenu({ ...menu, parentId: 1, parent: "" })
      : setMenu({ ...menu, parentId: 0, parent: "" });
  };
  const setStatusFlag = () => {
    ...
    <div>
      ...
      <div className="breadcrumb-header justify-content-between">
        <div className="left-content">
          <span className="main-content-title mg-b-0 mg-b-lg-1">Menu List</span>
        </div>
      </div>
      {/* <!-- /breadcrumb --> */}
                        ...
                        {menu.parentId === 0 ? (
                          ...
                          <FormGroup className="form-group">
                            <select
                              ...
                              name="parent"
                              value={menu.parent}
                              onChange={(event) => setMenuData(event)}>
                              {allData.data.map((e) => {
                                return (
                                  <option value={e.text} key={e.id}>
                                    {e.text}
                                  </option>
                                );
                              })}

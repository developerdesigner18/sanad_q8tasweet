...
const ElectionsDash = () => {
  ...
  React.useEffect(() => {
    fetch(backend_url + "getUpElection", { method: "GET" })
      ...
      .then((data) => {
        console.log("data1411", data);
        setAllData(data.data);
      });
    fetch(backend_url + "getPrev5Election", { method: "GET" })
      .then((response) => response.json())
      .then((data) => {
        console.log("prevelection", data);
        setPrev5Data(data.data);
      });
  }, []);
  return (
    <div>
                ...
                return (
                  <Col lg={6} xl={3} sm={6} key={element.id}>
                    <ElectionCard
                      ...
                      title={element.title}
                      image={element.image}
                      date={element.date}></ElectionCard>
                  </Col>
                );
              })}
          </Row>
              ...
              <div className="list-catergory1">
                <div className="item-list">
                  <ul className="list-group mb-0">
                    {prev5Data &&
                      prev5Data.map((element) => {
                        return (
                          <li
                            className="list-group-item d-flex border-bottom-0 pt-4"
                            key={element.id}>
                            <ElectionList
                              id={element.id}
                              title={element.title}

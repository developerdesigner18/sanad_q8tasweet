...
const RankList = () => {
      ...
      .then(async (data) => {
        ...
        await data.data.map(async (e) => {
          ...
          await data.data.map((ele) => {
            if (ele.id - e.parentid === 0) set_Data_row.parent_name = ele.name;
          });
  ...
  const addData = () => {
    ...
    fetch(backend_url + "addRank", requestOptions)
      .then((response) => response.json())
      .then((data) => {
  ...
  const updateData = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(backend_url + "updateRank", requestOptions)
          .then((response) => response.json())
          .then((data) => {
            if (data.code - 200 === 0) {
              getData(postData);
    ...
    <div>
      ...
      <div className="breadcrumb-header justify-content-between">
        <div className="left-content">
          <span className="main-content-title mg-b-0 mg-b-lg-1">Rank List</span>
        </div>
        <div className="justify-content-center mt-2">
          <Link
            ...
            onClick={() => {
              handleShow();
              setDataValue({ id: 0, name: "", permissionid: 0, parentid: 0 });
            }}>
            <i className="fe fe-plus me-2"></i>Add New Rank
          </Link>
          <Modal show={show.flag} onHide={handleClose}>
            <Modal.Header className="modal-header">
              {show.mode - 1 === 0 ? (
                <h6 className="modal-title">Add New Rank</h6>
              ) : (
                <h6 className="modal-title">Update Rank</h6>
              )}
              <Button
                variant=""
                className="btn-close"
                ...
                <Form className="form-horizontal">
                  ...
                  <FormGroup className="form-group">
                    <select
                      ...
                      className="form-control select2 form-select"
                      value={
                        dataValue.permissionid ? dataValue.permissionid : 0
                      }
                      onChange={(event) => setPropsValue(event)}>
                      <option value={0} key={""}>
                        No permission
                  ...
                  <FormGroup className="form-group">
                    <select
                      ...
                      name="parentid"
                      className="form-control select2 form-select"
                      value={dataValue.parentid ? dataValue.parentid : 0}
                      onChange={(event) => setPropsValue(event)}>
                      <option value={0} key={""}>
                        No parent
                      </option>

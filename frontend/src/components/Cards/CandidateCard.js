...
const CandidateCard = (props) => {
  ...
  React.useEffect(() => {
    fetch(
      ...
      .then((data) => {
        console.log("getGuaranteesCount", data);
        setGuaranteesCount(data.data);
      });
    fetch(backend_url + "getUserTeamCount/?id=" + props.userid, {
      method: "GET",
    })
      .then((response) => response.json())
      .then((data) => {
  ...
  return (
    <Card className="custom-card border">
      <Card.Body className="user-lock text-center">
        ...
        />
        <h4 className="fs-16 mb-0 mt-3 text-dark fw-semibold">{props.name}</h4>
